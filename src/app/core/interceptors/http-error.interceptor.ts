      ...
      catchError((error: HttpErrorResponse | any) => {
        if (request.url.startsWith(AppConstant.API_HOST)) {
          if (error instanceof HttpErrorResponse) {
            ...
            const statusCodeExclude = request.headers.getAll(
              AppConstant.HTTP_STATUS_CODE_EXCLUDE
            );
            if (
              statusCodeExclude &&
              statusCodeExclude.find((code) => +code === error.status)
            ) {
